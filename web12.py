...
# ========================================
    ...
    creator_profile = [
        {
            ...
            "Lecturer" : "DR. Edwin Setiawan Nugraha, S.Si., M.Sc.",
    ...
    for i, member in enumerate(creator_profile):
        # 3 kolom dengan foto di tengah
        col1, col2, col3 = st.columns([1, 1, 1])
        
        with col2:  # Foto di kolom tengah
            try:
                # Coba load foto lokal
                if os.path.exists(member["photo"]):
                    st.image(member["photo"], width=250)
                else:
                    # Fallback ke placeholder jika foto tidak ada
                    st.image("https://via.placeholder.com/250x250.png?text=" + member["name"].replace(" ", "+"), 
                    width=250)
            except:
                # Fallback jika error
                st.image("https://via.placeholder.com/250x250.png?text=" + member["name"].replace(" ", "+"), 
        ...
        <div class="member-card">
            ...
            <p style="color: #FFD700 !important;"><strong>Lecturer:</strong> {member["Lecturer"]}</p>
